...
class User extends \common\components\ARModel
{
    ...
    {
        return $this->hasOne(AdminUser::className(), ['id' => 'tem_id']);
    }
    ...
    public static function getUserOfflineData($idArr = [])
    {
        ...
        foreach ($idArr as $k => $arr) {
            $order              = Order::find()->where([
                ...
                'user_id',
                $arr,
            ])->andWhere('fee > 0')->select('SUM(deposit) AS deposit, SUM(fee) AS fee')->one();
            $data['amount'][$k] = $order->deposit ?: 0;
            $data['fee'][$k]    = $order->fee ?: 0;
            $data['rebate'][$k] = UserRebate::find()->where([
                'in',
                'user_id',
                $arr,
            ])->andWhere(['pid' => u()->id])->select('SUM(amount) AS amount')->one()->amount ?: 0;
        }
    ...
    {
        return UserCharge::find()->where([
            'charge_state' => UserCharge::CHARGE_STATE_PASS,
            'user_id'      => $user_id,
        ])->select('SUM(amount) AS amount')->one()->amount ?: 0;
    }
