...
class Product extends \common\components\ARModel
{
    ...
    public function rules()
    {
        return [
            ...
            [['deposit', 'fee'], 'number'],
            [['one_profit', 'force_sell', 'currency', 'hot', 'type', 'on_sale', 'state'], 'integer'],
     ...
     * @return boolean
     */
    public static function isTradeTime($productId, $early = 0)
    {
        $model = self::findOne($productId);
        $time = $model->trade_time;
        if (!$time) {
    ...
    // Map method of field `hot`
    public static function getHotMap($prepend = false)
    {
        $map = [
            self::HOT_YES => '是',
            self::HOT_NO => '否'
    ...
    // Format method of field `hot`
    public function getHotValue($value = null)
    {
    ...
    public static function getCurrencyMap($prepend = false)
    {
        $map = [
            ...
            self::CURRENCY_RHKD => '港元',
            self::CURRENCY_EURO => '欧元'
        ];
    ...
    {
        $map = [
            self::BUY_TYPE_CASH => '国内',
            self::BUY_TYPE_VOLUME => '国外'
        ];
