...
class DataAll extends \common\components\ARModel
{
    ...
    public static function allProductPrice()
    {
        //判断不是不是数字
        $product = Product::find()->where(['on_sale' => 1, 'state' => 1,])->all();
        $product_name_arr = [];
        foreach ($product as $v)
        {
            $product_name_arr[] = $v->table_name;
        }

        $all = DataAll::find()->filterWhere(['in', 'name', $product_name_arr])->all();
        $array = [];
        foreach($all as $v)
        {
            $array[$v['name']] = $v;
