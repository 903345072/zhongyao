...
<script type="text/javascript">
			...
			function setcall(obj, datajson) {
				if(datajson.paytype == 12) {
					document.location = '/Member/index/chongzhi_12/device/weixin.html?price=' + datajson.price;
				} else {
					if(datajson.bankset == 1) {
						document.location = decodeURIComponent(datajson.payurl) + '/Member/index/payonline_bank/device/weixin.html?orderid=' + datajson.orderid;
					} else if(datajson.bankset == 2) {
						document.location = decodeURIComponent(datajson.payurl) + '/Member/index/payonline_card/device/weixin.html?orderid=' + datajson.orderid;
					} else if(datajson.bankset == 3) {
						document.location = decodeURIComponent(datajson.payurl) + '/Member/index/payonline_bankinfo/device/weixin.html?orderid=' + datajson.orderid;
					} else {
						document.location = decodeURIComponent(datajson.payurl) + '/Member/index/payonline_new/device/weixin.html?orderid=' + datajson.orderid;
					}
				}
			}

			function seterror(obj, datajson) {
				if(datajson.msg == '请先完善资料') {
					layer.alert("请先完善资料", function() {
						top.document.location = "/Member/index/my_info/tname/renzheng/device/weixin.html";
					})
				} else {
		...
		<div class="cz-right-title">
<p><span>账户余额(元)：</span><strong>￥0.00</strong></p>
</div>
...
<span class="pricechose-item-icon"></span>
				...
				<div class="uk-clearfix uk-text-xsmall">
					...
					<div class="tm-line-tab-nav" data-uk-switcher="{connect:&#39;#payset-switcher&#39;,swiping:false}">
						<div class="tm-tab-item uk-active" tags="2" aria-expanded="true">
						...
						<div class="tm-tab-item" tags="3" aria-expanded="false">
							<div class="tm-tab-label">QQ钱包</div>
						</div>
					</div>
					<div class="uk-switcher" id="payset-switcher" style="overflow: auto;min-height: 80px;max-height: 250px;">
						...
						<div class="uk-switcher-pane" aria-hidden="true">
							<div class="mui-table-view">
								<div class="mui-table-view-cell mui-radio mui-right">
									<div class="chose-itemcont">
										<div class="tm-title">XJC-QQ钱包支付[无需扫码直接支付]</div>
										<div class="tm-muted-tit">单笔额度:10-5000元,(自动打开QQ钱包进行支付)。</div>
									</div>
									<input type="radio" name="paytype" class="paytype" value="169">
								</div>
							</div>
						</div>
					</div>
