...
function ajaxpost_form(obj) {
    ...
    if (before_function) {
        ...
        if (!returndata.status) {
            ...
            layer.msg(returndata.msg, {shade: [0.5, '#000000'], shadeClose: true});
            return false;
        }
    }
...
$(function () {
            ...
            if (datajson.status == 1) {
                ...
                if (datajson.msg) {
                    if (ajax_error_callback) {
                        layer.alert(datajson.msg, {icon: 2}, function () {
        ...
        $.post(page_set_url, datajson, function (datajson) {
            if (datajson.status == 1) {
                obj.pagination(datajson.data.totalitem, {
                    num_edge_entries: 2, num_display_entries: 4, callback: function (page_index, jq) {
    ...
    $('body').on("click", ".dialog_common", function () {
        ...
        layer.open({type: 2, title: title, shadeClose: true, shade: 0.8, area: ['50%', '40%'], content: dialogurl,});
        return false;
    })
    ...
    $("body").on("click", ".ajaxpost_form", function () {
        ...
        if (isconfirm) {
            layer.closeAll('loading');
            layer.confirm(isconfirm, {btn: ['取消', '确定'], title: confirmtitle,}, function () {
                layer.closeAll();
            }, function () {
                ajaxpost_form(obj);
            });
