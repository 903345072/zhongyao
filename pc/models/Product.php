...
class Product extends \common\models\Product
{
    ...
    {
        $products = self::find()->where([
            'on_sale' => self::ON_SALE_YES,
            'state'   => self::STATE_VALID,
        ])->limit(3)->orderBy('hot DESC')->all();
        $arr      = [];
    ...
    {
        $lists = self::find()->where([
            'on_sale' => self::ON_SALE_YES,
            'state'   => self::STATE_VALID,
            'type'    => self::BUY_TYPE_CASH,
        ])
            ->orderBy('hot DESC')
            ->with('dataAll')
            ->all();
    ...
    {
        $products          = self::find()->with(['dataAll'])->where([
            'on_sale' => self::ON_SALE_YES,
            'state'   => self::STATE_VALID,
            'id'      => $id,
        ])->orderBy('hot DESC')->one();
        ...
        if ($tradeTime) {
            ...
            $products['tradeTime'] = date('H:i', strtotime($tradeTime_) - 80);
        } else {
